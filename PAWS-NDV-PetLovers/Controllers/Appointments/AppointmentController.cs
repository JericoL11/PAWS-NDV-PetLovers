...
namespace PAWS_NDV_PetLovers.Controllers.Appointments
{
    public class AppointmentController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("AppointId", "OwnerId")] Appointment appointment)
        {
            ...
            if (checkOwner)
            {
                var appointmentOwner = _context.Appointments.Include(o => o.Owner).FirstOrDefault( o => o.OwnerId == appointment.OwnerId);

                if (appointmentOwner != null)
                {
                    ModelState.AddModelError("", $"The Owner ID \"{appointmentOwner.AppointId} - {appointmentOwner.Owner.fname} {appointmentOwner.Owner.lname}\" was already registered");
                }
                var ownersViewModel = await GetOwnersAsync();
